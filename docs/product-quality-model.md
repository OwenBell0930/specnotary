...
- 是否把技术组件图冒充产品/信息架构
- 页面/入口是否说明其用户问题、读写对象和跳转；同一业务状态是否有唯一权威来源
- 被页面或流程消费的核心对象，是否有职责、唯一性、关系、事实类型、生命周期和历史/当前边界
...
- 重复提交、并发、跨角色交接是否有口径
- 跨对象或异步处理是否说明部分成功、冲突、乱序、已保存事实、重试与恢复
...
Draft 的推荐组织顺序是：产品边界与复用基线 → 对象契约 → 页面/消费者视图 → 功能契约 → 异常恢复、权限与验收。它是起草方法，不是另一套评分模型。
