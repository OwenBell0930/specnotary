...
它不是完整商业 PRD 百科，也不是研发实施书或测试计划。它让产品经理在需求评审前把范围、产品/信息架构、角色、状态、交互、业务数据、错误定义、默认值、假设、冲突和未决摊开。写作由 Draft Skill 起草，质检分两层：独立 Product Review（产品质量模型）+ Structure Gate（CLI 确定性结构）。用户只输入原料、确认结果、拿报告去评审。已有文档可跳过 Draft，只走 Review + Gate。
