...
- **原型哈希是背书声明**：原型不会被 SpecNotary 重新生成；`generated_from_spec.hash` 只表示「有人声明原型对应此版机读」。因此 `sync` 默认**不**刷新它——机读变更后原型哈希保持 stale FAIL，直到人/Agent 复核原型并显式执行 `sync --attest-prototype`。背书是动作，不是副作用。
...
**`KILL_RATE 100%` 的分母是「已登记的变异」，不是「所有关键坏规格」。** 矩阵满分之后仍可能发现新缺口（YAML 重复键、空壳 title、模板占位词、behavior 无 name 等），发现后补进矩阵。同理，`tests/test_doc_consistency.py` 也曾漏过 CLI help、跨行注释、「自动刷新」措辞变体——门禁的能力边界，必须用它要求别人遵守的标准来陈述。

结论：**这两个机制的价值是防止已知覆盖退步，不是证明覆盖完整。** 机制守住已登记底线；未登记的坏规格要靠继续补变异，不能拿满分当完整覆盖。
