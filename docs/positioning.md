...
一句话：**SpecNotary 帮产品经理把需求原料整理成评审就绪的标准文档，并检查原料、方案、文档与已拍板载体的可选原型是否对得上。** 产品/信息架构、数据契约与错误定义是标准产品内容；原型是否制作及载体在接收原料后的首轮确认。先在试用页点按钮；感兴趣后把 GitHub 网址发给 Cursor 或 Codex，请它安装。规格写在她正在用的文件夹里即可，不必把 SpecNotary 设成工作区。之后只提供原料、确认结果、拿材料去评审。
...
| 「评审内容够具体」保证 | 空话 then/AC、占位 ui/defaults、断引用 → FAIL | 依赖模型与人自觉 | 依赖人自觉 | 无 |
...
- **中游**：Skill/LLM 先确认 `D-PROTOTYPE`，再按 `skills/specnotary/SKILL.md` 起草机读 YAML、人读，以及用户明确选择的原型载体（CLI 不解析自然语言）。
...
- 不产出技术栈、实现任务、测试策略、覆盖率或交付证据；这些属于评审后的研发与测试环节。
