...
- **对外入口**：产品经理在试用页点按钮即可；感兴趣后把项目发给 AI 助手安装。产品经理只面对原料、确认、评审材料。不覆盖多人在线改稿。
- **维护边界**：机读装载（`spec_io`）/ 规则与证据（`libspec`）/ 人读渲染（`spec_render`）分文件；Schema 对象数组与变异家族、顶层键、形状消毒表互相发现遗漏，不再只靠手写白名单。
...
- **全局视角人读（阅读动线，当前渲染器 v11）**：对照优秀解决方案说明书的阅读动线重建——目录、概览、范围、功能说明、架构总览、职责边界、数据契约、错误码、决策记录；门禁对账用的 HTML 注释放在文末；人读把文案键、状态枚举、业务动作、角色 ID 展开为中文（「单个文件不能超过 50MB」而不是 file_too_large；已终止而不是 terminated）；原料附录的处理结果也用中文；状态图按对象分组且不连箭头；主路径是功能说明的验收写法。口径见 [`docs/human-view.md`](docs/human-view.md)。
...
- **人读裸 ID（2026-08-19）**：当时只展开了 `empty_states` 和带前缀的 lifecycle id，正文仍会出现 `terminated` / `succeeded` / `created`。改为按机读标签全量展开（含 lifecycle 后缀别名、`value_labels`、无下划线的角色 id、默认值列表、控件显示条件、字段说明），并用测试钉住；禁止只改人读 Markdown。
- **输出自检报告给人看（2026-08-19，渲染器 v11）**：报告标题改为「输出自检报告」；结构检查两档写成「必须改 / 需要你拍板」，不再只甩 WARN；处理结果、落到说明书哪一段、结论都用中文，机读编号放括号里对账。试用页检查结果同步改成同一套说法。
- **使用方式收口**：产品经理路径是试用页 + 把本机文件夹交给 Cursor / Codex；GitHub Action、MCP 标为非产品路径，不做团队线上协同。
- **假详细两层**：词表补「形成闭环」「治理体系」「赋能」「抓手」等口号搭配；词表漏网（「本模块治理到位」）由助手按 Skill 用语义拦住，禁止写进硬检查的结构通过。
- **第五轮外部复评（2026-08-14）——证据单调性与证人真实性**：ready 删除 `sources[].path` 不再绕过快照（缺 path 必须比错误哈希更严）；原型自报 manifest 不再替代真实文件落点（属性位 `data-spec-id`，`<script>` 字符串不计；source/claim ID 不能给 UI 控件背书）；YAML 与 JSON 重复键同等拒绝；嵌套非对象 item / 非 UTF-8 人读稳定 FAIL；人读头部 `body_hash` / `gate_mode` 不得伪造；空话规则改为已知词表（不再宣传一般「不可观察性」），并校准 `待补充` / `智能识别` 误报；旗舰订单样例把源外推断标成 assumption，不再把「已支付不回券」写成与原料一致。渲染器升至 v6（人读 chrome 与证明口径对齐）。
...
- **图形语义（状态集合不连线）**：状态图改为**不连线的状态集合**——此前按声明顺序连箭头，在订单样例里画出了业务上不存在的 `completed --> cancelled`；教训是文字标注「这不是流程图」救不了错误的图形语义。
...
- **越界表述收紧**：README 与人读渲染文案改为「**已登记**原料条目的下落，账本完整性须人工抽查」；`SECURITY.md` 补 MCP 本地文件信任边界（不联网 ≠ 读取范围受限）；`proof-boundary.md` 补「`KILL_RATE` 100% 的分母是已登记变异，不是完整覆盖」——第三轮在满分状态下仍打出新缺口，全部已补入矩阵（含解析期变异）。
