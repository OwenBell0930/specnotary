...
- **增长面（发布前建设）**：`check --json` 机读判定；`human --lang en` 英文人读（中文输出逐字节不变，防漂按记录语言重渲染）；`specanvil precommit` + `.pre-commit-hooks.yaml`；GitHub Action（`action.yml`，PR 按文件标注）；`specanvil mcp` stdio server（Agent 可调用门禁，Experimental）；Pyodide Playground（浏览器零安装试用，已本地实测）；英文 README；VHS 动图脚本；英文空话词表校准（seamless / intuitive / user-friendly 等）。
