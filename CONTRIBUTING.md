...
感谢关注 SpecAnvil。SpecAnvil 的核心承诺是：**门禁只做确定性判定，不假装读懂自然语言**。所有贡献都不得破坏这一点。
...
2. **机读 Schema / 规则语义变化必须同步三处**：`src/specanvil/schemas/`、`docs/gate-modes.md`、README 能力表。
...
| 层 | 位置 | 效力 |
|----|------|------|
| JSON Schema | `src/specanvil/schemas/` | 类型/枚举/必填 |
