...
name: specnotary
description: >
  Use when the user works on SpecNotary / 规格工程 / machine-first specs.
---
...
3. Before claiming PASS: run `specnotary check` (or `./cli/run-check.sh`) — **FAIL must be 0**; WARN should be cleared or explicitly accepted.
...
```bash
specnotary check <machine.yaml>
specnotary human <machine.yaml> <human.md>
specnotary report <machine.yaml> [report.md]
```
